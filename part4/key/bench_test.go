...
func BenchmarkRSADecryption(b *testing.B) {
	sourceDate, label, privateKey := prepareRSA()
	publicKey := &privateKey.PublicKey
	md5hash := md5.New()
	encrypted, _ := rsa.EncryptOAEP(md5hash, rand.Reader, publicKey, sourceDate, label)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
...
func BenchmarkAESEnbryption(b *testing.B) {
